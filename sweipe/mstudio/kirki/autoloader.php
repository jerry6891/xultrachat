...
if ( ! function_exists( 'kirki_autoload_classes' ) ) {
	...
	function kirki_autoload_classes( $class_name ) {
		...
		if ( 0 === stripos( $class_name, 'Kirki' ) ) {
			...
			$path     = get_template_directory() . DIRECTORY_SEPARATOR . 'mstudio' . DIRECTORY_SEPARATOR . 'kirki' . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR;
			$filename = 'class-' . strtolower( str_replace( '_', '-', $class_name ) ) . '.php';

			$paths[] = $path . $filename;
			$paths[] = get_template_directory() . DIRECTORY_SEPARATOR . 'mstudio' . DIRECTORY_SEPARATOR . 'kirki' . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'lib' . DIRECTORY_SEPARATOR . $filename;
