<article id="post-<?php the_ID(); ?>" <?php post_class('tc-blog-item clearfix'); ?>>
	...
	<header class="entry-header">
		...
		<small class="subtitle">
			<span><?php esc_html_e('Posted', 'sweipe') ?></span>
			<?php sweipe::getPostDate() ?>
			<?php sweipe::getPostAuthor() ?>
			<?php sweipe::getPostCategories() ?>
			<?php sweipe::getPostTags() ?>
		</small>
	</header><!-- .entry-header -->
	...
	<div class="entry-content">
		...
		if( !is_single() && $post->post_excerpt ){
			...
			the_content( sprintf(
				esc_html__( 'Continue reading %s', 'sweipe' ),
	...
	<?php if ( !is_single() ) : ?>
		<div class="text-right">
			<a href="<?php the_permalink() ?>"><?php esc_html_e( 'Go to Post', 'sweipe' ) ?> <i class="uiicon-arrow413"></i></a>
		</div>
	<?php endif; ?>
	...
	<footer class="entry-footer">
		<?php edit_post_link( esc_html__( 'Edit', 'sweipe' ), '<span class="edit-link">', '</span>' ); ?>
	</footer><!-- .entry-footer -->

</article><!-- #post-## -->

<div class="tc-divider dark thick"></div>